...
// Index signature allows dynamic string key access (e.g. errors[doc.field])
...
export class CustomerApplyAccountComponent implements OnInit {
  ...
  private validateStep4(): boolean {
    ...
    required.forEach(([field, msg]) => {
      if (!this.docs[field]) { this.errors[field] = msg; ok = false; }
    });
    return ok;
  }
  ...
  onFileSelect(event: Event, field: keyof Step4Docs, imageOnly = false): void {
    ...
    if (!allowed.includes(file.type)) {
      this.errors[field] = `Only ${imageOnly ? 'JPG/PNG' : 'PDF/JPG/PNG'} allowed`;
      return;
    }
    if (file.size > maxMB * 1024 * 1024) {
      this.errors[field] = `Max ${maxMB}MB allowed`;
      return;
    }
